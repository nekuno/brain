...
{
    protected static $defaultName = 'neo4j:load-questions';
    ...
    public function __construct(LoggerInterface $logger, GraphManager $graphManager, UserManager $userManager, QuestionManager $questionManager)
    {
        ...
        foreach ($csv as $csvQuestion) {
            ...
            if (!is_null($keyQuestion)) {
                ...
                if (is_null($allQuestion['text_es']) || is_null($allQuestion['text_en'])) {
                    ...
                    try {
                        $this->questionManager->update($question_es);
                        $this->questionManager->update($question_en);
                        $updated += 1;
                ...
                try {
                    $question = $this->questionManager->create($question_es);
                    $question_en = array(
